...
/*   Updated: 2025/03/21 17:09:09 by andrean          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (word[++i])
	{
		...
		else if (word[i] == CHAR_DOLLAR)
		{
			...
			if (strcmp(var_name, "") == 0)
				ft_strncat_stop(&new_word, "$", 1);
			env_value = ft_get_env_value(var_name, env_ht);
			free(var_name);
			if (env_value)
				ft_strncat_stop(&new_word, env_value, ft_strlen(env_value));
		}
		else
			ft_strncat_stop(&new_word, &word[i], 1);
	}
	return (new_word);
}
...
void	ft_replace(char **new_word, char *word, int *i, t_hashtable *env_ht)
{
	...
	if (value)
		ft_strncat_stop(new_word, value, ft_strlen(value));
	else
		ft_strncat_stop(new_word, "", 0);
}
